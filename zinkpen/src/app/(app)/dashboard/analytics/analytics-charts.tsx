...
export function AnalyticsCharts({
  ...
  usage: UsageSnapshot;
  series: { day: string; words: number; images: number }[];
    ...
    <>
      ...
      <div className="space-y-6 p-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard label="Words generated" value={formatNumber(usage.wordsUsed)} sub="this billing cycle" icon={PenLine} />
          <StatCard label="Visuals created" value={`${usage.imagesUsed}`} sub="carousels, cards, ads" icon={ImageIcon} tone="gold" />
          <StatCard label="Documents" value={`${usage.documents}`} sub="across all projects" icon={FileText} tone="violet" />
          <StatCard label="Active seats" value={`${usage.seatsUsed}/${usage.seatsLimit}`} sub={`${planName} plan`} icon={Sparkles} tone="success" />
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={featureUsage} dataKey="value" nameKey="name" innerRadius={56} outerRadius={88} paddingAngle={3}>
